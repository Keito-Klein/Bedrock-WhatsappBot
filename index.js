...
async function sock() {
  ...
  client.ev.on('groups.update', (update) => {
		for (const n of update) {
			if (global.db.groupMetadata[n.id]) {
				groupCache.set(n.id, n);
				Object.assign(global.db.groupMetadata[n.id], n);
			}
		}
	});
  ...
  client.ev.on("messages.upsert", async (chatUpdate) => {
    ...
    if (mek.category === "peer") return;
      if (mek.key.id.startsWith("BAE5") && mek.key.id.length === 16) return;
		...
		if (store.messages[remoteJid].array.length > (setting.chatLength || 250)) {
  ...
  client.ev.on("group-participants.update", async (event) => {
    ...
    if (global.db && global.db.groupMetadata) {
...
`;
  ...
  client.ev.on("connection.update", async (update) => {
    ...
    if (connection === "close") {
      ...
      metadata = await client.groupMetadata("120363321807611707@g.us");
      if (!global.db.groupMetadata) global.db.groupMetadata = {};
      global.db.groupMetadata[metadata.id] = metadata;
    }
  });
